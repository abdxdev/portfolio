...
export async function GET(request: NextRequest) {
  ...
  if (response.status === 204) {
    ...
    console.error(`Failed to trigger update-readme workflow: ${response.status} ${text}`);
  }

  const redirectUri = request.nextUrl.searchParams.get("redirect_uri") ?? "https://github.com/abdxdev";
