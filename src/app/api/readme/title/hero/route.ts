...
function timeline(samples: [number, number][], T: number): { kt: string; v: string } {
  // normalise & sort
  let pts = samples.map(([t, v]) => [t / T, v] as [number, number])
...
function buildSvg(items: string[], theme: Theme): string {
  ...
  const W      = Math.ceil((maxLen + 2) * CHAR_W) + (LINE_MRG + LINE_GAP + 32) * 2
  const cx     = W / 2
  const cy     = H / 2
  const ly     = cy - 1
...
<circle cx="${fp(rxo-2,1)}" cy="${fp(ly,1)}" r="2" fill="${theme.accent}" opacity="0">
  ...
  const textEls = items.map((item, i) => {
    ...
    const glitch = frames.map((frame, f) => {
      ...
      return `<text x="${fp(cx,1)}" y="${fp(cy+1,1)}" text-anchor="middle" dominant-baseline="central" font-family="${FONT}" font-size="${FS}" font-weight="500" letter-spacing=".03em" fill="${theme.accent}" opacity="0">${esc(frame)}<animate attributeName="opacity" values="${ig.v}" keyTimes="${ig.kt}" ${IA}/><animate attributeName="opacity" values="${lg.v}" keyTimes="${lg.kt}" ${LA}/></text>`
    }).join('\n')
